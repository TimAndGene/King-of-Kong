...
Quintus["2D"] = function(Q) {
 ...
 Q.TileLayer = Q.Sprite.extend({
    ...
    collidableTile: function(tileNum) {
      //return tileNum != 2 && tileNum > 0;
      return tileNum !=2 && tileNum > 0;
    },


    ladderPresent: function(tileX,tileY) {
        //return this.p.tiles[tileY] && this.ladderTile(this.p.tiles[tileY][tileX]);
        return this.p.tiles[tileY] && this.ladderTile(this.p.tiles[tileY][tileX]);
    },

    ladderTile: function(tileNum) {
         return tileNum == 2;
    },
    ...
    collide: function(obj) {
      var p = this.p,
          ...
          ladder = this.collisionLadder;

      normal.collided = false;
      ladder.collided = false;
      ...
      for(var tileY = tileStartY; tileY<=tileEndY; tileY++) {
        ...
        if(this.ladderPresent(tileX,tileY)) {
            colObj.p.x = tileX * p.tileW + p.x + p.tileW;
            colObj.p.y = tileY * p.tileH + p.y + p.tileH;

            col = Q.collision(obj,colObj);

            if(col && col.magnitude > 0 &&
               (!ladder.collided || normal.magnitude < col.magnitude ) && UPBUTTON) {
                ladder.collided = true;
                normal.collided = true;
               // alert("holy shit");
      ...
      return normal.collided ? normal : false && ladder.collided ? ladder : false;
    },
  ...
  Q.gravityY = 9.8*100; // replace with *100
  Q.gravityX = 0;
  ...
  Q.component('2d',{
    ...
    collision: function(col,last) {
      ...
      if(col.normalX > 0.3) { 
        ...
        if(this.collisionLadder) {
            //alert("ladder has been touched");
            entity.trigger("ladder.on",col);
        }
    },
